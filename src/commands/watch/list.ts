...
export function configureListSubcommand(subcommand: SlashCommandSubcommandBuilder): SlashCommandSubcommandBuilder {
  return subcommand
...
export async function handleListSubcommand(base: WatchBase, interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < displayRows.length; i += 1) {
      ...
      try {
        ...
        logger.error({ err: error, uuid: record.watch_uuid }, "Failed to fetch watch details for list");
      }
      ...
      try {
        ...
        logger.warn({ err: error, uuid: record.watch_uuid }, "Failed to fetch watch history for list");
        if (!errorMessage) {
    ...
    logger.debug({ userId: interaction.user.id, count: rows.length, mode }, "/watch list completed");
  } catch (error: any) {
