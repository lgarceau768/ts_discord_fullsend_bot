...
export function configureAddSubcommand(subcommand: SlashCommandSubcommandBuilder): SlashCommandSubcommandBuilder {
  return subcommand
    ...
    .addStringOption((option) =>
      option
        .setName("url")
        .setDescription("Product/website URL")
        .setRequired(true),
    )
    .addStringOption((option) =>
      option
        .setName("title")
        .setDescription("Optional title to use for this watch"),
    )
    .addStringOption((option) =>
      option
        .setName("store")
...
export async function handleAddSubcommand(base: WatchBase, interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    if (!/^([a-z]+):\/\//i.test(base.notificationUrl)) {
      logger.warn({ notificationUrl: base.notificationUrl }, "Notification URL appears invalid (missing scheme)");
    }
