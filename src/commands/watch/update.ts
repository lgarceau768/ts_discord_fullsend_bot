...
export function configureUpdateSubcommand(subcommand: SlashCommandSubcommandBuilder): SlashCommandSubcommandBuilder {
  return subcommand
    ...
    .addStringOption((option) =>
      option
        .setName("title")
        .setDescription("New title to use for this watch"),
    )
    .addStringOption((option) =>
      option
        .setName("store")
        .setDescription("Store name (e.g., bestbuy, target, etc.)"),
    )
    .addStringOption((option) =>
      option
        .setName("tags")
    ...
    .addBooleanOption((option) =>
      option
        .setName("track_price")
        .setDescription("Track ld-json price data"),
    );
}

export async function handleUpdateSubcommand(base: WatchBase, interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    if (!record) {
      ...
      logger.warn({ userId: interaction.user.id, uuid }, "/watch update denied: not found or unauthorized");
      return;
    }
    ...
    if (store !== null || extraTagsInput !== null) {
      ...
      tagTitles = base.mkOwnerTags(interaction.user.id, `${interaction.user.username}#${interaction.user.discriminator ?? "0000"}`, store, extras);
    }
