...
export async function createTag(title: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Create tag "${title}" failed: ${res.status} ${JSON.stringify(json)}`);
...
export async function createWatch(opts: CreateWatchOpts): Promise<string> {
  ...
  const payload: any = {
    ...
    webdriver_delay: opts.fetchBackend === "html_webdriver" ? opts.webdriverDelaySec ?? 3 : undefined,
  ...
  if (!res.ok) {
    const message = json && Object.keys(json).length ? JSON.stringify(json) : text || res.statusText;
  ...
  try {
    ...
    if (!updateRes.ok) {
      ...
      logger.warn({ uuid, status: updateRes.status, body }, "Failed to enforce restock_diff processor via PUT");
    }
  } catch (error) {
...
export async function updateWatch(uuid: string, opts: UpdateWatchOpts): Promise<void> {
  ...
  if (opts.trackLdjsonPriceData !== undefined) payload.track_ldjson_price_data = opts.trackLdjsonPriceData;
  ...
  if (opts.intervalMinutes !== undefined) payload.time_between_check = { minutes: opts.intervalMinutes };
  ...
  if (!res.ok) {
    ...
    logger.error({ uuid, status: res.status, body: text, payload }, "ChangeDetection update failed");
...
export async function getWatchDetails(uuid: string): Promise<ChangeDetectionWatchDetails> {
  const base = requireUrl();
  const res = await fetch(`${base}/api/v1/watch/${encodeURIComponent(uuid)}`, { headers: headers() });
  const text = await res.text();
...
export async function getWatchHistory(uuid: string): Promise<ChangeDetectionHistoryEntry[]> {
  const base = requireUrl();
  const res = await fetch(`${base}/api/v1/watch/${encodeURIComponent(uuid)}/history`, { headers: headers() });
  const text = await res.text();
  let json: unknown;
  try {
    json = JSON.parse(text);
