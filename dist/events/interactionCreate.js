        ...
        if (interaction.isButton()) {
            ...
            if (kind === "tv") {
                ...
                const menu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                    ...
                    .addOptions({ label: "All seasons", value: "all" }, { label: "First season only", value: "first" }, { label: "Latest season only", value: "latest" }));
                await thread.send({
        ...
        if (interaction.isStringSelectMenu() && interaction.customId.startsWith("seasonpick:")) {
            ...
            try {
                ...
                const total = Array.isArray(details.seasons)
                    ...
                    : 0;
                let seasons = choice === "all" ? Array.from({ length: total }, (_, i) => i + 1)
                    : choice === "latest" ? [Math.max(1, total)]
                        : [1];
                if (!seasons.length)
                    seasons = pickDefaultSeasons(total);
