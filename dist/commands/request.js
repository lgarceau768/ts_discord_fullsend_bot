...
function parseSeasons(input, total) {
    if (!input)
        return null;
    const v = input.trim().toLowerCase();
    if (v === "all" || v === "first" || v === "latest")
        return v;
    const parts = v.split(",").map(s => Number(s.trim())).filter(n => Number.isInteger(n) && n > 0 && n <= Math.max(1, total));
    if (!parts.length)
        return null;
    // dedupe + sort
...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .setMinValue(1)
        .setMaxValue(10))
        .addStringOption(opt => opt.setName("seasons")
    ...
    async execute(interaction) {
        ...
        const entry = (threadId && getForThread(threadId)) ||
            getForChannel(interaction.channelId);
        if (!entry) {
            await interaction.editReply("No cached search results found here. Run `/search` first (it creates a thread and stores results).");
            return;
        }
        ...
        try {
            if (item.type === "movie") {
                ...
                const total = Array.isArray(details.seasons)
                    ...
                    : 0;
                let choice = parseSeasons(seasonsInput, total);
                let seasons;
                if (choice === "all") {
                ...
                await interaction.editReply(`✅ Requested **${item.title}** seasons ${seasons.join(", ")} (TMDB ${tmdb}).`);
            }
        }
        catch (e) {
